...
const TIME_SERVER = process.env.REACT_APP_TIME_SERVER_URL || "https://audio.ury.org.uk/webstudio/time";
...
export async function calculateOffset(): Promise<number> {
  ...
  for (let i = 0; i < ATTEMPTS; i++) {
    ...
    const t1 = parseFloat(line0) / 1e+6;
    const t2 = parseFloat(line1) / 1e+6;
    ...
    console.log(`Sync run ${i}; times ${t0} ${t1} ${t2} ${t3}; delta ${delta}; RTT ${rtt}`);
  ...
  console.log(`Clock sync complete, calculated offset ${offset}; RTT best/worst ${best[0][1]}/${best[ATTEMPTS - 1][1]}`)
  return offset;
}
